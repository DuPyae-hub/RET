    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormMessage({
          type: "error",
          text:
            data.message || data.error || "Failed to add default certificates",
        });
      }
    } catch (error) {
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Document{" "}
                  {selectedFile ? "(File Selected)" : "(Upload from Device)"}
                </label>
                <input
                  type="file"
                  accept="image/*,.pdf"
        ...
        {!loading && documents.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500">
              No legal documents yet. Add your first document above!
            </p>
          </div>
        )}
      </div>
